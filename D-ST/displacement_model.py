...
class PatchExpand(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.view(B, H, W, C)
        x = rearrange(
            x, 'b h w (p1 p2 c)-> b (h p1) (w p2) c', p1=2, p2=2, c=C // 4)
        x = x.view(B, -1, C // 4)
        x = self.norm(x)
...
class PatchEmbed(nn.Module):
    def __init__(self, img_size=128, patch_size=4, in_chans=2, embed_dim=96, norm_layer=None):
        super().__init__()
...
class SwinTransformerSys(nn.Module):
    def __init__(self, img_size=128, patch_size=4, in_chans=2, num_classes=2,
                 embed_dim=128, depths=[2, 4, 6], num_heads=[4, 8, 16],
        ...
        self.layers = nn.ModuleList()#新建一个空的 ModuleList，用来装每一层 BasicLayer（也就是一个 stage）
        for i_layer in range(self.num_layers):
            layer = BasicLayer(dim=int(embed_dim * 2 ** i_layer),
                               ...
                               downsample=PatchMerging if (
                                   i_layer < self.num_layers - 1) else None,#如果不是最后一层，就添加 PatchMerging 层进行下采样（分辨率变为 1/2，通道变为 2×）
                               use_checkpoint=use_checkpoint)
            self.layers.append(layer)

        self.layers_up = nn.ModuleList()
        self.concat_back_dim = nn.ModuleList()#每一层解码时都会 concat 编码器的 skip-connection 结果
        for i_layer in range(self.num_layers):
            #将 concat 后的通道数压缩回目标维度，#第0层不需要压缩，nn.Identity() 占位。
            concat_linear = nn.Linear(2 * int(embed_dim * 2 ** (self.num_layers - 1 - i_layer)),
                                      int(embed_dim * 2 ** (
                                          self.num_layers - 1 - i_layer))) if i_layer > 0 else nn.Identity()
            if i_layer == 0:#是最顶层（最右边），分辨率最小，要对输出进行上采样，恢复为更大分辨率
                ...
                layer_up = BasicLayer_up(dim=int(embed_dim * 2 ** (self.num_layers - 1 - i_layer)),
                                         ...
                                         upsample=PatchExpand if (
                                             i_layer < self.num_layers - 1) else None,#没到最后一层就用PatchExpand
                                         use_checkpoint=use_checkpoint)
            self.layers_up.append(layer_up)#保存构造好的模块
            self.concat_back_dim.append(concat_linear)

        self.norm = norm_layer(self.num_features)#编码器输出的归一化
        self.norm_up = norm_layer(self.embed_dim)#上采样后输出结果的归一化

        if self.final_upsample == "expand_first":
            #先把最后一层上采样4倍
            self.up = FinalPatchExpand_X4(input_resolution=(self.patches_resolution[0], self.patches_resolution[1]),
